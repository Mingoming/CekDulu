...
async function persistMemorySources({ retrieval, claim, article }) {
...
export async function analyzeInput(text) {
  ...
  const memory = await buildMemoryContext(claim || analysisText);
  await persistMemorySources({ retrieval, claim, article });
